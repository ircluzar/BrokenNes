...
{
    public class APU_HI2X : IAPU
    {
        // Core metadata
        public string CoreName => "High Fidelity 2X";
        public string Description => "Based on the High Fidelity 2 core, this variant refines the warm/detail blend and adds a smooth, dreamy reverb tail.";
        public int Performance => -12;
        public int Rating => 5;
        public string Category => "Enhanced";
        private readonly Bus bus;
        public APU_HI2X(Bus bus) { this.bus = bus; }
        ...
        private const int AudioRingSize = 32768;
        private readonly float[] audioRing = new float[AudioRingSize];
        private int ringWrite, ringRead, ringCount; private double fractionalSampleAccumulator;

        // Filters state (detail path)
        private double hp90LastIn, hp90LastOut;
        private double hp440LastIn, hp440LastOut;
        private double lpLast;
        private static readonly double Hp90Coeff = Math.Exp(-2.0 * Math.PI * 90.0 / audioSampleRate);
        private static readonly double Hp440Coeff = Math.Exp(-2.0 * Math.PI * 440.0 / audioSampleRate);
        private static readonly double Lp14kCoeff = Math.Exp(-2.0 * Math.PI * 14000.0 / audioSampleRate);

        // Filters state (warm path)
        private double warmLpLast;
        private double warmBassLast;
        private static readonly double WarmLpCoeff = Math.Exp(-2.0 * Math.PI * 3200.0 / audioSampleRate);
        private static readonly double WarmBassCoeff = Math.Exp(-2.0 * Math.PI * 180.0 / audioSampleRate);
        private const float WarmMix = 0.32f;
        private const float WarmBassBoost = 0.28f;
        private const float DetailMix = 0.85f;

        // Reverb (smooth, faint)
        private const int ReverbDelayMs = 120;
        private static readonly int ReverbDelaySamples = (audioSampleRate * ReverbDelayMs) / 1000;
        private readonly float[] reverbBuffer = new float[ReverbDelaySamples];
        private int reverbIndex;
        private float reverbDampState;
        private const float ReverbMix = 0.12f;
        private const float ReverbFeedback = 0.35f;
        private const float ReverbDamp = 0.78f;

        // Channel enables (bitmask: 0x01=p1, 0x02=p2, 0x04=triangle, 0x08=noise, 0x10=dmc)
        ...
        public void Step(int cpuCycles)
        {
            ...
            double sampleIncrement = audioSampleRate / CpuFreq; // samples per CPU cycle
        ...
        private static double HighPass(double x, ref double xPrev, ref double yPrev, double a)
        {
        ...
        private static double LowPass(double x, ref double yPrev, double a)
        {
            double y = (1.0 - a) * x + a * yPrev;
            yPrev = y;
            return y;
        }

        private float ApplyDetailFilterChain(float sample)
        {
            double x = sample;
            x = HighPass(x, ref hp90LastIn, ref hp90LastOut, Hp90Coeff);
            x = HighPass(x, ref hp440LastIn, ref hp440LastOut, Hp440Coeff);
            x = LowPass(x, ref lpLast, Lp14kCoeff);
            return (float)x;
        }

        private float ApplyWarmFilterChain(float sample)
        {
            double lp = LowPass(sample, ref warmLpLast, WarmLpCoeff);
            double bass = LowPass(sample, ref warmBassLast, WarmBassCoeff);
            double boosted = lp + bass * WarmBassBoost;
        ...
        private float ApplyReverb(float sample)
        {
            ...
            float damped = reverbDampState + (delayed - reverbDampState) * (1.0f - ReverbDamp);
            reverbDampState = damped;
            float inputToBuffer = sample + damped * ReverbFeedback;
            reverbBuffer[reverbIndex] = inputToBuffer;
            reverbIndex++;
            if (reverbIndex >= reverbBuffer.Length) reverbIndex = 0;
            return sample + damped * ReverbMix;
        }
        ...
        private void MixAndStore()
        {
            ...
            int t = ChannelEnabled(0x04) ? triangle_output : 0; // 0..15
            int n = ChannelEnabled(0x08) ? noise_output : 0;    // 0..15
            int d = (ChannelEnabled(0x10) && dmc_enabled) ? dmc_output : 0; // 0..127
            // Index layout: (t << (4+7)) | (n << 7) | d
            int tndIndex = (t << 11) | (n << 7) | d;
            float tnd = TndMixLut[tndIndex];

            float mixed = pulseMix + tnd;
            float detail = ApplyDetailFilterChain(mixed) * DetailMix;
            float warm = ApplyWarmFilterChain(mixed) * WarmMix;
            float combined = (detail + warm) * 1.05f;
            float withReverb = ApplyReverb(combined);
            StoreSample(withReverb);
        }
            ...
            if(state is System.Text.Json.JsonElement je){
                try{
                    if(je.ValueKind==System.Text.Json.JsonValueKind.Object){
                        ...
                        if(je.TryGetProperty("hp90In", out v)) tmp.hp90In=v.GetDouble();
                        if(je.TryGetProperty("hp90Out", out v)) tmp.hp90Out=v.GetDouble();
                        if(je.TryGetProperty("hp440In", out v)) tmp.hp440In=v.GetDouble();
                        if(je.TryGetProperty("hp440Out", out v)) tmp.hp440Out=v.GetDouble();
                        if(je.TryGetProperty("lpLast", out v)) tmp.lpLast=v.GetDouble();
                        if(je.TryGetProperty("warmLpLast", out v)) tmp.warmLpLast=v.GetDouble();
                        if(je.TryGetProperty("warmBassLast", out v)) tmp.warmBassLast=v.GetDouble();
                        if(je.TryGetProperty("frac", out v)) tmp.frac=v.GetDouble();
                        if(je.TryGetProperty("reverbIndex", out v)) tmp.reverbIndex=v.GetInt32();
                        if(je.TryGetProperty("reverbDampState", out v)) tmp.reverbDampState=v.GetSingle();
                        if(je.TryGetProperty("reverbBuffer", out v) && v.ValueKind == System.Text.Json.JsonValueKind.Array)
                        {
                            int len = Math.Min(reverbBuffer.Length, v.GetArrayLength());
                            tmp.reverbBuffer = new float[reverbBuffer.Length];
                            for (int i = 0; i < len; i++)
                            {
                                tmp.reverbBuffer[i] = v[i].GetSingle();
                            }
                        }
