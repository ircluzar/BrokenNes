...
{
public sealed class CPU_ULQ : ICPU, IApuCycleScaler {
	// Metadata defaults
	public string CoreName => "Ultra Low Quality";
	public string Description => "Based on the LW2 core, this ULQ variant under-performs at a fixed 0.75x budget with added cycle drag.";
	...
	// ULQ cycle scaling: CPU executes at 0.75x effective throughput.
	private int cpuCycleRemainder;
	private int apuCycleRemainder;
	...
	public int ExecuteInstruction() {
		...
		if (exec != null) cycles = exec();
		else if (IgnoreInvalidOpcodes) cycles = 2; // treat unknown as NOP
		else throw new CpuCrashException($"Bad opcode {opcode:X2} at {(PC-1):X4}");
		return ScaleCpuCycles(cycles);
	}

	private int ScaleCpuCycles(int baseCycles)
	{
		cpuCycleRemainder += baseCycles;
		int extra = cpuCycleRemainder / 3;
		cpuCycleRemainder -= extra * 3;
		return baseCycles + extra;
	}

	public int ScaleApuCycles(int cpuCycles)
	{
		// Scale inflated CPU cycles back down to a 0.75x APU clock.
		apuCycleRemainder += cpuCycles * 9;
		int scaled = apuCycleRemainder / 16;
		apuCycleRemainder -= scaled * 16;
		return scaled;
	}
