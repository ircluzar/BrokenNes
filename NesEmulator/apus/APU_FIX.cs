...
namespace NesEmulator
{
    public class APU_FIX : IAPU
    {
        ...
        public APU_FIX(Bus bus) { this.bus = bus; }
        ...
        public void SetState(object state){
            if(state is ApuState s){ RestoreState(s); return; }
            if(state is System.Text.Json.JsonElement je){
                try{
                    if(je.ValueKind==System.Text.Json.JsonValueKind.Object){
                        // Build a temporary ApuState with only found fields; missing remain default
                        var tmp = new ApuState();
                        if(je.TryGetProperty("pulse1_duty", out var v)) tmp.pulse1_duty = (byte)v.GetByte();
                        ...
                        // Optional counters/flags (guard with TryGetProperty)
                        bool b; int iv; ushort us;
                        if(je.TryGetProperty("pulse1_enabled", out v)) { try{ b=v.GetBoolean(); tmp.pulse1_enabled=b; }catch{} }
                        if(je.TryGetProperty("pulse2_enabled", out v)) { try{ b=v.GetBoolean(); tmp.pulse2_enabled=b; }catch{} }
                        if(je.TryGetProperty("triangle_enabled", out v)) { try{ b=v.GetBoolean(); tmp.triangle_enabled=b; }catch{} }
                        if(je.TryGetProperty("noise_enabled", out v)) { try{ b=v.GetBoolean(); tmp.noise_enabled=b; }catch{} }
                        if(je.TryGetProperty("dmc_enabled", out v)) { try{ b=v.GetBoolean(); tmp.dmc_enabled=b; }catch{} }
                        if(je.TryGetProperty("p1Len", out v)) { try{ iv=v.GetInt32(); tmp.p1Len=iv; }catch{} }
                        if(je.TryGetProperty("p2Len", out v)) { try{ iv=v.GetInt32(); tmp.p2Len=iv; }catch{} }
                        if(je.TryGetProperty("tLen", out v)) { try{ iv=v.GetInt32(); tmp.tLen=iv; }catch{} }
                        if(je.TryGetProperty("nLen", out v)) { try{ iv=v.GetInt32(); tmp.nLen=iv; }catch{} }
                        if(je.TryGetProperty("noiseShift", out v)){ us=(ushort)v.GetUInt16(); tmp.noiseShift=us; }
                        if(je.TryGetProperty("noiseTimer", out v) && v.TryGetInt32(out iv)) tmp.noiseTimer=iv;
                        if(je.TryGetProperty("frameCycle", out v)) { try{ iv=v.GetInt32(); tmp.frameCycle=iv; }catch{} }
                        if(je.TryGetProperty("frameMode5", out v)) { try{ b=v.GetBoolean(); tmp.frameMode5=b; }catch{} }
                        if(je.TryGetProperty("frameIRQInhibit", out v)) { try{ b=v.GetBoolean(); tmp.frameIRQInhibit=b; }catch{} }
                        if(je.TryGetProperty("frameIRQFlag", out v)) { try{ b=v.GetBoolean(); tmp.frameIRQFlag=b; }catch{} }
                        if(je.TryGetProperty("lpLast", out v)) tmp.lpLast = (float)v.GetSingle();
                        if(je.TryGetProperty("dcLastIn", out v)) tmp.dcLastIn = (float)v.GetSingle();
                        if(je.TryGetProperty("dcLastOut", out v)) tmp.dcLastOut = (float)v.GetSingle();
                        if(je.TryGetProperty("ringWrite", out v) && v.TryGetInt32(out iv)) tmp.ringWrite=iv;
                        if(je.TryGetProperty("ringRead", out v) && v.TryGetInt32(out iv)) tmp.ringRead=iv;
                        if(je.TryGetProperty("ringCount", out v) && v.TryGetInt32(out iv)) tmp.ringCount=iv;
