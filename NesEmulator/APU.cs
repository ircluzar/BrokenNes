...
namespace NesEmulator
{
    ...
    private int nextFrameEventCycle = 7457; // next CPU cycle (relative to frameCycle) at which a quarter/half frame event occurs
        ...
        public void Step(int cpuCycles)
        {
            ...
            // Incremental sampling: advance APU one CPU cycle at a time and emit host samples
            // at the correct fractional rate so waveform transitions are captured (previous
            // implementation batched and duplicated final cycle state -> audible desync).
    ...
    private void ClockDMC()
        ...
        public void SetState(object state){ if(state is ApuState s){ RestoreState(s); return; } if(state is System.Text.Json.JsonElement je){ try{ var s2 = System.Text.Json.JsonSerializer.Deserialize<ApuState>(je.GetRawText(), new System.Text.Json.JsonSerializerOptions{IncludeFields=true}); if(s2!=null) RestoreState(s2);}catch{} } }
