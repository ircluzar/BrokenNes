...
{
    public class APU_WFLQ : IAPU
    {
        private Bus bus;
    public APU_WFLQ(Bus bus)
        {
            this.bus = bus;
        }
        ...
        // Program (General MIDI) suggestions (host may ignore & pick oscillator types instead)
        public int ProgramPulse1 { get; set; } = 80; // Lead 1 (square)
    ...
    public event Action<string,int>? PitchEvent; // (channel, bendValue -8192..8191)
        ...
        {
            if (activePulse1Note.HasValue) NoteEvent?.Invoke(new NesNoteEvent("P1", activePulse1Note.Value, 0, false, ProgramPulse1));
        ...
        private void EmitNoteOn(string ch, ref int? active, int midi, int vel, int program, double baseFreq = 0)
        {
            ...
            if (active.HasValue && active.Value == midi)
            {
        ...
        private void ProcessSoundFontNotes()
        {
            ...
            if (pulse1_enabled && pulse1_lengthCounter > 0 && pulse1_timer >= 8)
            {
                ...
                int vel = VolumeToVelocity(pulse1_constantVolume ? pulse1_volumeParam : pulse1_decayLevel);
                // Trigger if no active note, note number changed beyond bend range, or explicit attack
                if (!activePulse1Note.HasValue || p1Attack)
                {
                    lastP1Velocity = vel;
            ...
            {
                EmitNoteOff("P1", ref activePulse1Note, ProgramPulse1);
            }
            ...
            if (pulse2_enabled && pulse2_lengthCounter > 0 && pulse2_timer >= 8)
            {
                ...
                int targetNote = FreqToMidi(freq);
                int vel = VolumeToVelocity(pulse2_constantVolume ? pulse2_volumeParam : pulse2_decayLevel);
                if (!activePulse2Note.HasValue || p2Attack)
                {
                    lastP2Velocity = vel;
            ...
            {
                EmitNoteOff("P2", ref activePulse2Note, ProgramPulse2);
            }
            ...
            if (triangle_enabled && triangle_lengthCounter > 0 && triangle_timer >= 2)
            {
                ...
                {
                    EmitNoteOn("TRI", ref activeTriangleNote, targetNote, 90, ProgramTriangle, freq);
                }
                else
                {
            ...
            if (noise_enabled && noise_lengthCounter > 0)
            {
                ...
                if (trigger)
                {
                    ...
                    int rawVel = VolumeToVelocity(noise_constantVolume ? noise_volumeParam : noise_decayLevel);
                    int vel = (int)Math.Round(rawVel * 0.6); // tame noise loudness
                    if (vel < 1) vel = 1;
