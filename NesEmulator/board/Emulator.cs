...
namespace BrokenNes
{
    ...
    // LoadBenchHistory now in Benchmark partial
        ...
        private async Task TriggerAchievementUnlockFlowAsync(string id, string title)
        {
            ...
            try
            {
                // Hard stop new frames immediately to avoid races before any awaits
                try { nesController.IsRunning = false; _activeClock?.Stop(); } catch { }
                // 1) Pause emulation immediately
                try { await PauseEmulation(); } catch { }
                // 2) Manually advance exactly one frame while paused to settle state
                try { await StepOneFrameForAchievementsAsync(); } catch { }
                // 3) Save state to the default slot (this also persists the paired achievements snapshot)
                ...
                // 4) Register achievement in game save (by unique ID) and mark level cleared
                ...
                // 4b) Mark trusted DeckBuilder continue for this rom
                try
                {
                ...
                // 5) Open modal with title and id; 6) Auto-redirect after 5s
                _achModalId = id;
        ...
        // Run a single frame while paused to settle achievement state, then rotate RA windows
        private async Task StepOneFrameForAchievementsAsync()
        {
            try
            {
                if (nes == null) return;
                // Use current inputs, run exactly one frame without clock
                try { nes.SetInputs(inputState, inputStateP2); } catch { }
                nes.RunFrame();
                // Drain any audio generated by this frame and flush JS-side output
                try { var _ = nes.GetAudioBuffer(); await JS.InvokeVoidAsync("nesInterop.flushAudioOutput"); } catch { }
                // Update framebuffer to avoid stale canvas if any draw occurs while paused
                ...
                // Rotate RA RAM snapshots and internal counters; ignore returned unlocks
                try { _achEngine?.EvaluateFrame(); } catch { }
            }
            catch { }
        }
