...
namespace NesEmulator
{
    ...
    //THIS IS A BACKUP OF THE PREVIOUS APU WHICH HAD ISSUES. DO NOT DELETE.
    
    public class APUJANK : IAPU
    {
        private Bus bus;
        public APUJANK(Bus bus)
        {
            this.bus = bus;
        }
        ...
        // ====== SAVE STATE SUPPORT (placed mid-class for clarity) ======
        private class ApuState
        {
            ...
            public double fractionalSampleAccumulator; public int ringWrite, ringRead, ringCount; // ring meta only (not raw buffer)
        }
        public object GetState() => new ApuState
        {
            pulse1_duty = pulse1_duty,
            pulse1_length = pulse1_length,
            pulse1_sweep = pulse1_sweep,
            pulse1_timer = pulse1_timer,
            pulse2_duty = pulse2_duty,
            pulse2_length = pulse2_length,
            pulse2_sweep = pulse2_sweep,
            pulse2_timer = pulse2_timer,
            triangle_linear = triangle_linear,
            triangle_length = triangle_length,
            triangle_timer = triangle_timer,
            noise_length = noise_length,
            noise_period = noise_period,
            status = status,
            pulse1_sequenceIndex = pulse1_sequenceIndex,
            pulse2_sequenceIndex = pulse2_sequenceIndex,
            pulse1_timerCounter = pulse1_timerCounter,
            pulse2_timerCounter = pulse2_timerCounter,
            triangle_sequenceIndex = triangle_sequenceIndex,
            triangle_timerCounter = triangle_timerCounter,
            triangle_linearCounter = triangle_linearCounter,
            triangle_linearReloadFlag = triangle_linearReloadFlag,
            noise_timerCounter = noise_timerCounter,
            noiseShiftRegister = noiseShiftRegister,
            pulse1_envelopeStart = pulse1_envelopeStart,
            pulse1_envelopeDivider = pulse1_envelopeDivider,
            pulse1_decayLevel = pulse1_decayLevel,
            pulse2_envelopeStart = pulse2_envelopeStart,
            pulse2_envelopeDivider = pulse2_envelopeDivider,
            pulse2_decayLevel = pulse2_decayLevel,
            noise_envelopeStart = noise_envelopeStart,
            noise_envelopeDivider = noise_envelopeDivider,
            noise_decayLevel = noise_decayLevel,
            pulse1_constantVolume = pulse1_constantVolume,
            pulse1_volumeParam = pulse1_volumeParam,
            pulse1_lengthHalt = pulse1_lengthHalt,
            pulse2_constantVolume = pulse2_constantVolume,
            pulse2_volumeParam = pulse2_volumeParam,
            pulse2_lengthHalt = pulse2_lengthHalt,
            noise_constantVolume = noise_constantVolume,
            noise_volumeParam = noise_volumeParam,
            noise_lengthHalt = noise_lengthHalt,
            triangle_lengthHalt = triangle_lengthHalt,
            pulse1_output = pulse1_output,
            pulse2_output = pulse2_output,
            triangle_output = triangle_output,
            noise_output = noise_output,
            pulse1_lengthCounter = pulse1_lengthCounter,
            pulse2_lengthCounter = pulse2_lengthCounter,
            triangle_lengthCounter = triangle_lengthCounter,
            noise_lengthCounter = noise_lengthCounter,
            pulse1_sweepDivider = pulse1_sweepDivider,
            pulse1_sweepNegate = pulse1_sweepNegate,
            pulse1_sweepShift = pulse1_sweepShift,
            pulse1_sweepPeriod = pulse1_sweepPeriod,
            pulse1_sweepReload = pulse1_sweepReload,
            pulse2_sweepDivider = pulse2_sweepDivider,
            pulse2_sweepNegate = pulse2_sweepNegate,
            pulse2_sweepShift = pulse2_sweepShift,
            pulse2_sweepPeriod = pulse2_sweepPeriod,
            pulse2_sweepReload = pulse2_sweepReload,
            frameSequencerMode5 = frameSequencerMode5,
            frameIRQInhibit = frameIRQInhibit,
            frameSequenceStep = frameSequenceStep,
            cyclesToNextFrameEvent = cyclesToNextFrameEvent,
            lpLast = lpLast,
            dcLastIn = dcLastIn,
            dcLastOut = dcLastOut,
            pulse1_enabled = pulse1_enabled,
            pulse2_enabled = pulse2_enabled,
            triangle_enabled = triangle_enabled,
            noise_enabled = noise_enabled,
            fractionalSampleAccumulator = fractionalSampleAccumulator,
            ringWrite = ringWrite,
            ringRead = ringRead,
            ringCount = ringCount
        };
        ...
        private void ApplyState(ApuState s)
        {
            ...
            pulse1_constantVolume = s.pulse1_constantVolume; pulse1_volumeParam = s.pulse1_volumeParam; pulse1_lengthHalt = s.pulse1_lengthHalt;
            pulse2_constantVolume = s.pulse2_constantVolume; pulse2_volumeParam = s.pulse2_volumeParam; pulse2_lengthHalt = s.pulse2_lengthHalt;
            noise_constantVolume = s.noise_constantVolume; noise_volumeParam = s.noise_volumeParam; noise_lengthHalt = s.noise_lengthHalt;
            triangle_lengthHalt = s.triangle_lengthHalt; pulse1_output = s.pulse1_output; pulse2_output = s.pulse2_output; triangle_output = s.triangle_output; noise_output = s.noise_output;
        ...
        {
            // Recompute channel cached outputs for consistent first sample
            ...
            // Clear ring buffer to avoid mixing old audio from pre-load state (prevents glitch/noise burst)
            ringRead = ringWrite = ringCount = 0; lpLast = 0; dcLastIn = 0; dcLastOut = 0;
        }
        private void ResetAudio()
        {
            Array.Fill(audioRing, 0.0f);
            ringWrite = 0;
            ringRead = 0;
            ringCount = 0;
            fractionalSampleAccumulator = 0;
            pulse1_sequenceIndex = pulse2_sequenceIndex = 0;
            triangle_sequenceIndex = 0;
            pulse1_timerCounter = pulse2_timerCounter = 0;
            triangle_timerCounter = 0;
            noise_timerCounter = 0;
            pulse1_lengthCounter = pulse2_lengthCounter = triangle_lengthCounter = noise_lengthCounter = 0;
            pulse1_envelopeStart = pulse2_envelopeStart = noise_envelopeStart = false;
            pulse1_envelopeDivider = pulse2_envelopeDivider = noise_envelopeDivider = 0;
            pulse1_decayLevel = pulse2_decayLevel = noise_decayLevel = 0;
            pulse1_sweepDivider = pulse2_sweepDivider = 0;
            frameSequenceStep = 0;
            cyclesToNextFrameEvent = 3729;
            noiseShiftRegister = 1;
            lpLast = 0f;
        }

        // Step 1 CPU cycle worth of APU time
        public void Step()
        {
            Step(1);
        }

        // Batched stepping to avoid high interop overhead; 'cpuCycles' CPU cycles elapsed
        public void Step(int cpuCycles)
        {
            const double CpuFreq = 1789773.0;
            // Run per CPU cycle updates for timers/sequence to avoid frequency glide
            ...
            double samplesFloat = (double)cpuCycles * audioSampleRate / CpuFreq;
        ...
        {
            // If ring full, drop oldest (avoid stalls/gaps). NES audio is continuous; better to keep most recent.
            ...
            // Convert current channel integer outputs into NES mixing formula domain
            ...
            // Soft clip just a bit
            mixed = (float)Math.Tanh(mixed * 2.2f);
            // Low-pass filter for mild alias smoothing
            lpLast += (mixed - lpLast) * LowPassCoeff;
            float filtered = lpLast;
            // DC blocker (first-order high-pass): y[n] = x[n] - x[n-1] + R*y[n-1]
            const float R = 0.995f;
            float hp = filtered - dcLastIn + R * dcLastOut;
            dcLastIn = filtered;
            dcLastOut = hp;
            // Apply global volume attenuation.
            // Originally unity; sequence of attenuations applied:
            // -5% => 0.95
            // -7% => 0.95 * 0.93 = 0.8835
            // Additional -10% requested => 0.8835 * 0.90 = 0.79515 overall.
            audioRing[ringWrite] = hp * 0.79515f;
        ...
        {
            new byte[8]{0,1,0,0,0,0,0,0}, // 12.5%
            new byte[8]{0,1,1,0,0,0,0,0}, // 25%
            new byte[8]{0,1,1,1,1,0,0,0}, // 50%
            new byte[8]{1,0,0,1,1,1,1,1}, // 75% (inverted 25%)
        };
        ...
        public void WriteAPURegister(ushort address, byte value)
        {
            switch (address)
            {
                case 0x4000: // Pulse 1 duty, envelope
                    pulse1_duty = (byte)((value >> 6) & 0x03);
                    pulse1_lengthHalt = (value & 0x20) != 0; // also envelope loop
                    pulse1_constantVolume = (value & 0x10) != 0;
                    pulse1_volumeParam = value & 0x0F;
                    pulse1_envelopeStart = true;
                    pulse1_enabled = true;
                    break;
                case 0x4001: // Pulse 1 sweep
                    pulse1_sweep = value;
                    pulse1_sweepPeriod = ((value >> 4) & 0x07) + 1;
                    pulse1_sweepNegate = (value & 0x08) != 0;
                    pulse1_sweepShift = value & 0x07;
                    pulse1_sweepReload = true;
                    break;
                case 0x4002: // Pulse 1 timer low
                    pulse1_timer = (ushort)((pulse1_timer & 0xFF00) | value);
                    break;
                case 0x4003: // Pulse 1 timer high, length
                    pulse1_timer = (ushort)((pulse1_timer & 0x00FF) | ((value & 0x07) << 8));
                    pulse1_length = (byte)((value >> 3) & 0x1F);
                    if (pulse1_length < LengthTable.Length)
                        pulse1_lengthCounter = LengthTable[pulse1_length];
                    pulse1_sequenceIndex = 0;
                    pulse1_envelopeStart = true;
                    pulse1_timerCounter = pulse1_timer + 1;
                    break;

                case 0x4004: // Pulse 2 duty, envelope
                    pulse2_duty = (byte)((value >> 6) & 0x03);
                    pulse2_lengthHalt = (value & 0x20) != 0;
                    pulse2_constantVolume = (value & 0x10) != 0;
                    pulse2_volumeParam = value & 0x0F;
                    pulse2_envelopeStart = true;
                    pulse2_enabled = true;
                    break;
                case 0x4005: // Pulse 2 sweep
                    pulse2_sweep = value;
                    pulse2_sweepPeriod = ((value >> 4) & 0x07) + 1;
                    pulse2_sweepNegate = (value & 0x08) != 0;
                    pulse2_sweepShift = value & 0x07;
                    pulse2_sweepReload = true;
                    break;
                case 0x4006: // Pulse 2 timer low
                    pulse2_timer = (ushort)((pulse2_timer & 0xFF00) | value);
                    break;
                case 0x4007: // Pulse 2 timer high, length
                    pulse2_timer = (ushort)((pulse2_timer & 0x00FF) | ((value & 0x07) << 8));
                    pulse2_length = (byte)((value >> 3) & 0x1F);
                    if (pulse2_length < LengthTable.Length)
                        pulse2_lengthCounter = LengthTable[pulse2_length];
                    pulse2_sequenceIndex = 0;
                    pulse2_envelopeStart = true;
                    pulse2_timerCounter = pulse2_timer + 1;
                    break;

                case 0x4008: // Triangle linear counter
                    triangle_linear = value;
                    triangle_lengthHalt = (value & 0x80) != 0; // control flag also length counter halt
                    triangle_linearReloadFlag = true;
                    triangle_enabled = true;
                    break;
                case 0x400A: // Triangle timer low
                    triangle_timer = (ushort)((triangle_timer & 0xFF00) | value);
                    break;
                case 0x400B: // Triangle timer high, length
                    triangle_timer = (ushort)((triangle_timer & 0x00FF) | ((value & 0x07) << 8));
                    triangle_length = (byte)((value >> 3) & 0x1F);
                    if (triangle_length < LengthTable.Length)
                        triangle_lengthCounter = LengthTable[triangle_length];
                    triangle_sequenceIndex = 0;
                    triangle_linearReloadFlag = true;
                ...
                case 0x400C: // Noise envelope
                    noise_lengthHalt = (value & 0x20) != 0;
                    noise_constantVolume = (value & 0x10) != 0;
                    noise_volumeParam = value & 0x0F;
                    noise_envelopeStart = true;
                    noise_enabled = true;
                    break;
                ...
                case 0x400F: // Noise length
                    noise_length = value;
                    int nIdx = (value >> 3) & 0x1F;
                    if (nIdx < LengthTable.Length)
                        noise_lengthCounter = LengthTable[nIdx];
                    noise_envelopeStart = true;
                    break;

                case 0x4015: // Status register
                    status = value;
                    if ((value & 0x01) == 0) pulse1_enabled = false;
                    if ((value & 0x02) == 0) pulse2_enabled = false;
                    if ((value & 0x04) == 0) triangle_enabled = false;
                    if ((value & 0x08) == 0) noise_enabled = false;
                    break;
                case 0x4017: // Frame sequencer
                    frameSequencerMode5 = (value & 0x80) != 0; // we still primarily use 4-step
        ...
        // Pull up to maxSamples from ring buffer. If maxSamples <=0, return all available (bounded by a safeguard)
        public float[] GetAudioSamples(int maxSamples = 0)
        {
            ...
            // Prevent gigantic arrays if caller forgets to bound
            if (toRead > 4096 && maxSamples <= 0) toRead = 4096;

            float[] result = new float[toRead];
            int first = Math.Min(toRead, AudioRingSize - ringRead);
            Array.Copy(audioRing, ringRead, result, 0, first);
            int remaining = toRead - first;
            if (remaining > 0)
            {
                Array.Copy(audioRing, 0, result, first, remaining);
            }
            ringRead = (ringRead + toRead) & (AudioRingSize - 1);
            ringCount -= toRead;
            if (ringCount < 0) ringCount = 0; // safety
            return result;
        }
        ...
        {
            // Envelopes
            ClockEnvelope(ref pulse1_envelopeStart, ref pulse1_envelopeDivider, ref pulse1_decayLevel, pulse1_volumeParam, pulse1_lengthHalt);
            ClockEnvelope(ref pulse2_envelopeStart, ref pulse2_envelopeDivider, ref pulse2_decayLevel, pulse2_volumeParam, pulse2_lengthHalt);
            ClockEnvelope(ref noise_envelopeStart, ref noise_envelopeDivider, ref noise_decayLevel, noise_volumeParam, noise_lengthHalt);

            // Triangle linear counter
            if (triangle_linearReloadFlag)
            {
                triangle_linearCounter = triangle_linear & 0x7F;
            }
            else if (triangle_linearCounter > 0)
            {
                triangle_linearCounter--;
            }
            if ((triangle_linear & 0x80) == 0)
            {
                triangle_linearReloadFlag = false;
            }
        }

        private void HalfFrameTick()
        {
            // Length counters (unless halt set)
            ...
            // Sweeps
            DoSweep(ref pulse1_timer, pulse1_sweepNegate, pulse1_sweepShift, pulse1_sweepPeriod, ref pulse1_sweepDivider, ref pulse1_sweepReload);
            DoSweep(ref pulse2_timer, pulse2_sweepNegate, pulse2_sweepShift, pulse2_sweepPeriod, ref pulse2_sweepDivider, ref pulse2_sweepReload);
        }

        private void DoSweep(ref ushort timer, bool negate, int shift, int period, ref int divider, ref bool reload)
        {
            if (shift == 0 || period == 0) { if (reload) reload = false; return; }
            if (reload)
            {
                divider = period;
                reload = false;
            ...
            else
            {
                ...
                int target = negate ? (timer - change - 1) : (timer + change);
        ...
        private void ClockEnvelope(ref bool start, ref int divider, ref int decayLevel, int volumeParam, bool loop)
        {
            ...
            else
            {
                ...
                {
                    divider = volumeParam + 1;
                    if (decayLevel > 0) decayLevel--; else if (loop) decayLevel = 15;
                }
            }
        }
        ...
        {
            cyclesToNextFrameEvent--;
            if (cyclesToNextFrameEvent > 0) return;
            // Fire event
            switch (frameSequenceStep)
            {
                case 0: // 3729: quarter
                    QuarterFrameTick();
                    cyclesToNextFrameEvent = 3728; // to 7457
                    break;
                case 1: // 7457: quarter + half
                    QuarterFrameTick();
                    HalfFrameTick();
                    cyclesToNextFrameEvent = 3729; // to 11186
                    break;
                case 2: // 11186: quarter
                    QuarterFrameTick();
                    cyclesToNextFrameEvent = 2730; // to 14916
                    break;
                case 3: // 14916: quarter + half (then restart)
                    QuarterFrameTick();
                    HalfFrameTick();
                    cyclesToNextFrameEvent = 3729; // restart sequence (total 14916+3729=18645 ~ 4*60Hz)
                    break;
            }
        ...
        private void ClockPulse(ref ushort timer, ref int counter, ref int seqIndex)
        {
            ...
            if (counter <= 0)
            {
                // Each pulse sequencer step advances every (timer+1)*2 CPU cycles on NES
                counter += (timer + 1) * 2;
                seqIndex = (seqIndex + 1) & 7;
                // Update outputs now so sample generator sees stable values
                pulse1_output = ComputePulseOutput(1);
                pulse2_output = ComputePulseOutput(2);
            }
        }
        ...
        private int ComputePulseOutput(int which)
        {
            if (which == 1)
            {
                if (!pulse1_enabled || pulse1_lengthCounter == 0 || pulse1_timer < 8) return 0;
                var pattern = PulseDutyTable[pulse1_duty & 3];
                int bit = pattern[pulse1_sequenceIndex];
                int vol = pulse1_constantVolume ? pulse1_volumeParam : pulse1_decayLevel;
                return bit == 1 ? vol : 0;
            }
            else
            {
                if (!pulse2_enabled || pulse2_lengthCounter == 0 || pulse2_timer < 8) return 0;
                var pattern = PulseDutyTable[pulse2_duty & 3];
                int bit = pattern[pulse2_sequenceIndex];
                int vol = pulse2_constantVolume ? pulse2_volumeParam : pulse2_decayLevel;
                return bit == 1 ? vol : 0;
            }
        }
        ...
        {
            if (!triangle_enabled || triangle_lengthCounter == 0 || triangle_linearCounter == 0 || triangle_timer < 2) return;
            if (triangle_timerCounter > 0)
            {
                triangle_timerCounter--;
            }
            if (triangle_timerCounter <= 0)
            {
                triangle_timerCounter += (triangle_timer + 1);
                triangle_sequenceIndex = (triangle_sequenceIndex + 1) & 31; // 32-step
                triangle_output = TriangleSequenceValue(triangle_sequenceIndex);
            }
        }

        private int TriangleSequenceValue(int index)
        {
            // 32-step sequence: 15..0,0..15
            if (index < 16) return 15 - index; // 15..0
            else return index - 16;            // 0..15
        }

        private void ClockNoise()
        {
            if (!noise_enabled || noise_lengthCounter == 0) return;
            if (noise_timerCounter > 0) noise_timerCounter--;
            if (noise_timerCounter <= 0)
            {
                int periodIndex = noise_period & 0x0F;
                noise_timerCounter += NoisePeriods[periodIndex];
                int bit0 = noiseShiftRegister & 0x01;
                int bit1 = ((noise_period & 0x80) != 0) ? ((noiseShiftRegister >> 6) & 0x01) : ((noiseShiftRegister >> 1) & 0x01);
                int feedback = bit0 ^ bit1;
                noiseShiftRegister = (ushort)((noiseShiftRegister >> 1) | (feedback << 14));
                int vol = noise_constantVolume ? noise_volumeParam : noise_decayLevel;
