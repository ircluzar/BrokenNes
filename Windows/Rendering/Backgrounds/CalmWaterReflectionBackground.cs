using System;
using SharpDX;
using SharpDX.Direct2D1;
using SharpDX.DXGI;
using SharpDX.Mathematics.Interop;

namespace BrokenNes.Windows.Rendering
{
    /// <summary>
    /// Calm water reflection with gentle ripples
    /// </summary>
    public class CalmWaterReflectionBackground : IBackground
    {
        private const int Width = 64;
        private const int Height = 48;
        
        private SharpDX.Direct2D1.Bitmap? bitmap;
        private double time = 0.0;
        private double timeSinceLastUpdate = 0.0;
        private const double UpdateInterval = 0.04;
        
        public void Update(double deltaTime)
        {
            time += deltaTime * 0.2;
            timeSinceLastUpdate += deltaTime;
        }
        
        public void Render(RenderTarget renderTarget, RawRectangleF destRect)
        {
            if (bitmap != null)
            {
                renderTarget.DrawBitmap(bitmap, destRect, 0.5f, BitmapInterpolationMode.Linear);
            }
        }
        
        public void Initialize(RenderTarget renderTarget, int clientWidth, int clientHeight)
        {
            RegenerateTexture(renderTarget);
        }
        
        private void RegenerateTexture(RenderTarget renderTarget)
        {
            if (timeSinceLastUpdate < UpdateInterval)
                return;
            
            timeSinceLastUpdate = 0.0;
            
            var pixelData = new byte[Width * Height * 4];
            float t = (float)time;
            
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    float nx = x / (float)Width;
                    float ny = y / (float)Height;
                    
                    // Multiple gentle ripples
                    float ripple1 = (float)Math.Sin(nx * 8.0f + t * 0.5f) * (float)Math.Cos(ny * 6.0f + t * 0.4f);
                    float ripple2 = (float)Math.Sin(nx * 12.0f - t * 0.3f) * (float)Math.Cos(ny * 10.0f - t * 0.35f);
                    float ripple3 = (float)Math.Sin(nx * 5.0f + t * 0.6f) * (float)Math.Cos(ny * 7.0f + t * 0.45f);
                    
                    float ripples = (ripple1 * 0.4f + ripple2 * 0.3f + ripple3 * 0.3f);
                    ripples = (ripples + 1.0f) * 0.5f; // Normalize
                    
                    // Depth gradient (darker at bottom)
                    float depth = ny * 0.3f;
                    
                    // Combine
                    float value = ripples * 0.7f + depth * 0.3f;
                    
                    // Turquoise water
                    float hue = 180.0f + value * 20.0f;
                    float saturation = 0.5f + value * 0.2f;
                    float lightness = 0.25f + value * 0.2f;
                    
                    var rgb = ColorMath.HslToRgb(hue, saturation, lightness);
                    
                    int offset = (y * Width + x) * 4;
                    pixelData[offset + 0] = rgb.b;
                    pixelData[offset + 1] = rgb.g;
                    pixelData[offset + 2] = rgb.r;
                    pixelData[offset + 3] = 255;
                }
            }
            
            bitmap?.Dispose();
            
            using (var stream = new DataStream(pixelData.Length, true, true))
            {
                stream.Write(pixelData, 0, pixelData.Length);
                stream.Position = 0;
                
                var props = new BitmapProperties(new PixelFormat(Format.B8G8R8A8_UNorm, SharpDX.Direct2D1.AlphaMode.Ignore));
                bitmap = new SharpDX.Direct2D1.Bitmap(renderTarget, new Size2(Width, Height), stream, Width * 4, props);
            }
        }
        
        public void Dispose()
        {
            bitmap?.Dispose();
            bitmap = null;
        }
        
        internal void UpdateTexture(RenderTarget renderTarget)
        {
            if (timeSinceLastUpdate >= UpdateInterval)
            {
                RegenerateTexture(renderTarget);
            }
        }
    }
}
