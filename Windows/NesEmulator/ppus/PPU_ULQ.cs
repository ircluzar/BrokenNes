...
{
// Ultra Low Quality PPU core ("ULQ")
// Heavier degradation pass inspired by very early consoles.
// The goal is a visibly coarse output while keeping timing/logic compatibility.
// Current degradations:
//  * Aggressive background fetch throttling and higher sprite penalty.
...
public class PPU_ULQ : IPPU
{
    ...
    public string Description => "Ultra-low-fidelity mode. Uses aggressive fetch limits while staying compatible.";
    ...
    // Degradation model:
    //  * Background fetch budget is heavily reduced.
    //  * Sprite penalty higher; scanline sprite limit lower.
    //  * DRAM stall probability higher.
    private const int BaseBackgroundFetchBudget = 228;
    private const int MinBackgroundFetchBudget = 208;
    private const int SpritePenaltyPerSprite = 4;
    private const int MaxSpritesPerScanlineULQ = 4;
    private const double DramRefreshSkipProbability = 0.04; // 4% chance
    private const int HorizontalBlockSize = 4;
    private const int VerticalBlockSize = 2;
    private readonly System.Random rand = new System.Random(0x51A70E);
    private int currentBgFetchBudget;
    private bool duplicateScanlineFromPrevious; // set when DRAM stall triggers.

    public PPU_ULQ(Bus bus)
    {
        this.bus = bus;
    ...
    private void RenderScanline(int y)
    {
        ...
        if (!bgEnabled && !sprEnabled)
        {
            ...
            ApplyUltraLowQualityScanline(y);
            return;
        }
    ...
    private void ApplyUltraLowQualityScanline(int y)
    {
        if (VerticalBlockSize > 1 && y > 0 && (y % VerticalBlockSize) != 0)
        {
            var fb = frameBuffer;
            if (fb == null) return;
            System.Buffer.BlockCopy(fb, (y - 1) * ScreenWidth * 4, fb, y * ScreenWidth * 4, ScreenWidth * 4);
            return;
        }
        QuantizeScanlineColors(y);
        ApplyHorizontalBlock(y);
    }
    ...
    private void ApplyHorizontalBlock(int y)
    {
        var fb = frameBuffer;
        if (fb == null) return;
        int baseIdx = y * ScreenWidth * 4;
        ...
        while (x < ScreenWidth)
        {
            int blockEnd = System.Math.Min(ScreenWidth, x + HorizontalBlockSize);
            int count = blockEnd - x;
            int rSum = 0, gSum = 0, bSum = 0;
            for (int i = x; i < blockEnd; i++)
            {
                int idx = baseIdx + i * 4;
                rSum += fb[idx]; gSum += fb[idx + 1]; bSum += fb[idx + 2];
            }
            byte r = (byte)(rSum / count); byte g = (byte)(gSum / count); byte b = (byte)(bSum / count);
            for (int i = x; i < blockEnd; i++)
            {
                int idx = baseIdx + i * 4;
                fb[idx] = r; fb[idx + 1] = g; fb[idx + 2] = b; fb[idx + 3] = 255;
    ...
    // Reduce precision: 1-1-1 bus with stronger attenuation.
    ...
    private void RenderSprites(int scanline, bool[] bgMask)
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            if (drawnThisLine >= MaxSpritesPerScanlineULQ) break;
    ...
    private void InitializeTestFrameBuffer()
    {
        ...
        for (int y = 0; y < ScreenHeight; y++)
        {
            ...
            ApplyUltraLowQualityScanline(y); // degrade pattern too
        }
    }
