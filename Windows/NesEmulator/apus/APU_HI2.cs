...
{
    public class APU_HI2 : IAPU
    {
        // Core metadata
        public string CoreName => "High Fidelity 2";
        public string Description => "Based on the High Fidelity core, this variant layers a warm low-passed path under the detailed path for smoother, fuller output.";
        public int Performance => -10;
        public int Rating => 5;
        public string Category => "Enhanced";
        private readonly Bus bus;
        public APU_HI2(Bus bus) { this.bus = bus; }
        ...
        private static readonly double WarmLpCoeff = Math.Exp(-2.0 * Math.PI * 3500.0 / audioSampleRate);
        private static readonly double WarmBassCoeff = Math.Exp(-2.0 * Math.PI * 200.0 / audioSampleRate);
        private const float WarmMix = 0.2f;
        private const float WarmBassBoost = 0.3f;
        private const float DetailMix = 0.5f;
