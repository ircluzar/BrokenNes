...
{
    /// <summary>
    /// Lorenz attractor visualization - chaotic but smooth flowing patterns
    /// </summary>
    public class LorenzAttractorBackground : IBackground
    {
        ...
        private float x = 0.1f, y = 0.0f, z = 0.0f;
        ...
        private void RegenerateTexture(RenderTarget renderTarget)
        {
            ...
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    ...
                    // Initialize local Lorenz state
                    float lx = nx * 10.0f + (float)Math.Sin(t * 0.5f);
                    float ly = ny * 10.0f + (float)Math.Cos(t * 0.5f);
                    float lz = 20.0f + (float)Math.Sin(t * 0.3f) * 5.0f;
                    ...
                    // Map to color
                    float value = (float)Math.Sqrt(lx * lx + ly * ly + lz * lz) / 50.0f;
                    value = Math.Clamp(value, 0.0f, 1.0f);
                    
                    // Deep teal to cyan
                    float hue = 170.0f + value * 30.0f;
                    float saturation = 0.6f + value * 0.2f;
                    float lightness = 0.15f + value * 0.35f;
