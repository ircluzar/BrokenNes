...
{
    /// <summary>
    /// Animated Julia set fractal with slowly changing parameters
    /// </summary>
    public class JuliaSetBackground : IBackground
    {
        ...
        private void RegenerateTexture(RenderTarget renderTarget)
        {
            ...
            // Slowly varying Julia set parameters
            float cx = -0.7f + (float)Math.Sin(t * 0.3f) * 0.15f;
            float cy = 0.27015f + (float)Math.Cos(t * 0.25f) * 0.1f;
            
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    float zx = (x / (float)Width - 0.5f) * 3.0f;
                    float zy = (y / (float)Height - 0.5f) * 3.0f;
                    ...
                    smooth = smooth / maxIter;
                    smooth = Math.Clamp(smooth, 0.0f, 1.0f);
                    
                    // Green to emerald gradient
                    float hue = 140.0f + smooth * 40.0f;
                    float saturation = 0.5f + smooth * 0.3f;
                    float lightness = 0.15f + smooth * 0.3f;
