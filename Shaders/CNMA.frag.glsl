...
// Cinematic, content-aware grading
// - Dynamic local exposure around midgray based on neighborhood luma
// - Midtone contrast curve with highlight rolloff
// - Subtle teal/shadow + warm/highlight color grade
// - Gentle local contrast (unsharp) and vignette
// - Optional very soft halation from bright regions
...
void main(){
  ...
  // Local luminance averages at two radii
  ...
  // Content-adaptive exposure toward a cinematic midgray
  ...
  // Midtone contrast curve around a dynamic pivot near the local average
  ...
  // Highlight rolloff to keep a filmic shoulder
  ...
  // Subtle teal & orange grade: cool shadows, warm highlights
  float L2 = luma(col2);
  ...
  // Adaptive saturation: boost midtones slightly, compress extremes
  ...
  // Very soft halation from bright regions (content-driven, not random)
