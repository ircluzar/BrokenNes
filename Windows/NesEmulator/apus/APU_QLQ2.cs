...
namespace NesEmulator
{
    ...
    private int ringWrite, ringRead, ringCount; private double sampleFrac; // legacy fractional pacing (kept for backward state)
    private const int SampleRate = 44100; 
    private double cpuFreq = 1789773.0; // NTSC default; PAL uses ~1662607.0
    private double samplesPerCpu = 44100.0 / 1789773.0; // recalculated on reset (legacy path)
    private const int CpuFreqInt = 1789773; // integer CPU clock for Bresenham pacing
    private int samplePhase; // accumulates +SampleRate per CPU cycle; emit while >= CpuFreqInt
    ...
    public APU_QLQ2(Bus bus)
        ...
        public void Step(int cpuCycles)
        {
            ...
            for (int c = 0; c < cpuCycles; c++)
            {
                ...
                // Integer Bresenham sample pacing
                samplePhase += SampleRate;
                if (samplePhase >= CpuFreqInt)
                {
    ...
    // (removed stray brace that prematurely closed class)
        ...
        private void MixAndStoreOutput()
        {
            ...
            float tndMix = TndMixLut[(t << 11) | (n << 7) | d];
            float mixed = (pulseMix + tndMix) * OutputGain;
            // Low-pass then proper DC high-pass using previous input & output
            lpLast += (mixed - lpLast) * LowPassCoeff; float lp = lpLast; float hp = lp - dcLastIn + DC_HPF_R * dcLastOut; dcLastIn = lp; dcLastOut = hp;
            audioRing[ringWrite] = hp;
    ...
    public void SetEnabledChannels(int channelMask) { channelEnableMask = channelMask & 0x1F; }
        ...
        public void ClearAudioBuffers()
        {
            ...
            sampleFrac = 0; samplePhase = 0; lpLast = 0; dcLastIn = 0; dcLastOut = 0;
        }
    ...
    // === Save / Load (expanded) ===
    private class State { public int v=4; public int oscEnables, frameMode, frameDelay, frame, framePeriod; public bool irqFlag; public double sampleFrac; public int samplePhase; public float lpLast, dcLastIn, dcLastOut; public QnSquare? s1,s2; public QnTriangle? tri; public QnNoise? noi; public QnDmc? d; public int ringWrite,ringRead,ringCount; public bool frame5StepMode, frameIrqInhibit; public bool nonlinear; public bool pal; public int nextIrqCycle, earliestIrqCycle, lastTime, lastDmcTime; }
    public object GetState()=> new State { oscEnables=oscEnables, frameMode=frameMode, frameDelay=frameDelay, frame=frame, framePeriod=framePeriod, irqFlag=irqFlag, sampleFrac=sampleFrac, samplePhase=samplePhase, lpLast=lpLast, dcLastIn=dcLastIn, dcLastOut=dcLastOut,
        ...
        public void SetState(object state){ 
            ...
            if(state is System.Text.Json.JsonElement je) {
                try {
                    ...
                    if(je.TryGetProperty("samplePhase", out p)) newState.samplePhase = p.GetInt32();
                    if(je.TryGetProperty("lpLast", out p)) newState.lpLast = p.GetSingle();
                    if(je.TryGetProperty("dcLastIn", out p)) newState.dcLastIn = p.GetSingle();
                    if(je.TryGetProperty("dcLastOut", out p)) newState.dcLastOut = p.GetSingle();
        ...
        private void ApplyState(State s) {
            oscEnables=s.oscEnables; frameMode=s.frameMode; frameDelay=s.frameDelay; frame=s.frame; framePeriod=s.framePeriod; irqFlag=s.irqFlag; sampleFrac=0; samplePhase=0; lpLast=0; dcLastIn=0; dcLastOut=0; square1 = s.s1; square2 = s.s2; triangle = s.tri; noise = s.noi; dmc = s.d; // clear any serialized audio backlog
    ...
    private static QnDmc Clone(QnDmc d){ return new QnDmc{ regs=(byte[])d.regs.Clone(), regWritten=(bool[])d.regWritten.Clone(), lastAmp=d.lastAmp, timerPeriod=d.timerPeriod, timerCounter=d.timerCounter, sampleAddress=d.sampleAddress, sampleLengthRemaining=d.sampleLengthRemaining, startAddress=d.startAddress, startLength=d.startLength, shiftReg=d.shiftReg, bitsRemaining=d.bitsRemaining, deltaCounter=d.deltaCounter, sampleBuffer=d.sampleBuffer, sampleBufferFilled=d.sampleBufferFilled, silence=d.silence, irqEnable=d.irqEnable, loop=d.loop, irqFlag=d.irqFlag, dac=d.dac, palMode=d.palMode, nonlinear=d.nonlinear, nextIrqCycle=d.nextIrqCycle }; }
        ...
        public void Reset(bool palMode, int initialDmcDac = 0)
        {
            ...
            lastTime = 0; lastDmcTime = 0; oscEnables = 0; irqFlag=false; frameDelay=1; frame=0; frameMode=0; nextIrqCycle=noIrq; earliestIrqCycle=noIrq; sampleFrac=0; samplePhase=0; lpLast=0; dcLastIn=0; dcLastOut=0; ringRead=ringWrite=ringCount=0;
