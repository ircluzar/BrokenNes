...
{
    public class APU_HI : IAPU
    {
        // Core metadata
        public string CoreName => "High Fidelity";
        public string Description => "Based on the Low Power core, this variant prioritizes clarity with higher sample rate and a more accurate analog-style filter chain.";
        public int Performance => -8;
        public int Rating => 5;
        public string Category => "Enhanced";
        private readonly Bus bus;
        public APU_HI(Bus bus) { this.bus = bus; }
        ...
        // Filters state (analog-style chain for cleaner output)
        ...
        private float ApplyFilterChain(float sample)
        {
            double x = sample;
        ...
        private void MixAndStore()
        {
            ...
            float filtered = ApplyFilterChain(mixed);
            StoreSample(filtered * 1.05f);
        }
