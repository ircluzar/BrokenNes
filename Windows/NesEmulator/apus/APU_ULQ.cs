...
{
    public class APU_ULQ : IAPU
    {
    // Core metadata
    public string CoreName => "Ultra Low Quality";
    public string Description => "Ultra-low-fidelity APU with coarse voice levels, reduced noise resolution, and 1-bit-ish DMC undersampling.";
    public int Performance => -25;
    public int Rating => 0;
    public string Category => "Degraded";
        private readonly Bus bus;
        public APU_ULQ(Bus bus) { this.bus = bus; }
        ...
        // the canonical NES mixing approximation exactly.
        ...
        // Filters state
        private float lpLast, dcLastIn, dcLastOut; private const float LowPassCoeff = 0.08f; private const float DC_HPF_R = 0.992f;

        // Channel enables (bitmask: 0x01=p1, 0x02=p2, 0x04=triangle, 0x08=noise, 0x10=dmc)
        private int channelEnableMask = 0x1F;

        // ULQ degradation parameters
        private const int DmcDownsampleFactor = 6;
        private const int OutputQuantizeLevels = 32;
        private int dmcDownsampleCounter;
        private int dmcHeldBit;
        ...
        private static readonly int[] NoisePeriods = { 8,16,24,32,48,64,96,128,160,192,224,256,320,384,512,768 };
        ...
        public void WriteAPURegister(ushort address, byte value)
        {
            switch(address)
            {
                ...
                case 0x4017: // frame counter
                    ...
                    frameCycle = 0; frameStep = 0; nextFrameEventCycle = 7457;
                    if(frameMode5){ QuarterFrameTick(); HalfFrameTick(); }
                    frameIRQFlag = false;
                    break;
            }
        }
        ...
        private void ClockNoise()
        {
            ...
            if(--noise_timerCounter <= 0){
                ...
                int tap = ((noise_periodReg & 0x80)!=0) ? ((noiseShiftRegister >> 5)&1) : ((noiseShiftRegister >> 1)&1);
                int fb = bit0 ^ tap;
                noiseShiftRegister = (ushort)((noiseShiftRegister >> 1) | (fb<<6));
                noiseShiftRegister &= 0x7F;
            }
        ...
        {
            if(dmc_sampleBufferFilled) return; if(dmc_sampleLengthRemaining==0) return;
        ...
        private void MixAndStore()
        {
            ...
            int pulseSum = QuantizeVoice(p1) + QuantizeVoice(p2);
            ...
            int tq = QuantizeVoice(t);
            int nq = QuantizeNoise(n);
            int dq = QuantizeDmc(d);

            int tndIndex = (tq << 11) | (nq << 7) | dq;
            ...
            StoreSample(QuantizeSample(hp * 1.05f));
        }

        private static int QuantizeVoice(int value)
        {
            if (value <= 0) return 0;
            int level = (value + 2) / 5;
            if (level > 3) level = 3;
        ...
        private static int QuantizeNoise(int value)
        {
            if (value <= 0) return 0;
            return value < 8 ? 7 : 15;
        }

        private int QuantizeDmc(int value)
        {
            if (dmcDownsampleCounter <= 0)
            {
                dmcHeldBit = value >= 64 ? 127 : 0;
                dmcDownsampleCounter = DmcDownsampleFactor;
            }
            dmcDownsampleCounter--;
            return dmcHeldBit;
        }

        private static float QuantizeSample(float sample)
        {
            if (sample > 1f) sample = 1f;
            if (sample < -1f) sample = -1f;
            float scaled = (sample + 1f) * 0.5f;
            float step = 1f / (OutputQuantizeLevels - 1);
            int q = (int)Math.Round(scaled / step);
            return (q * step) * 2f - 1f;
        }
        ...
        private void ResetInternal(){ ringRead=ringWrite=ringCount=0; fractionalSampleAccumulator=0; lpLast=dcLastIn=dcLastOut=0; frameCycle=0; frameIRQFlag=false; dmc_irqFlag=false; dmcDownsampleCounter=0; dmcHeldBit=0; }
